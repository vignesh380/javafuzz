...
class Helper {
        ...
        if (constant == null) {
            if (args.length > 0) {
                if (MethodArgs.length > 0) {
                    outArgs(args);
                    out(DoIt2(cons, mtd, args, MethodArgs, v));
                } else {
                    outArgs(args);
                    out(DoIt2(cons, mtd, args, new Double(1), v));
                }
            } else {
                if (MethodArgs.length > 0) {
                    outArgs(args);
                    out(DoIt2(cons, mtd, MethodArgs, new Integer(1), v));
                } else {
                    outArgs(args);
                    out(DoIt2(cons, mtd, v));
                }
            }
        } else {
            if (args.length > 0) {
                ...
                if (g == 1) {
                    for (int a = 0; a < args.length; a++) {
                        ...
                        if (MethodArgs.length > 0) {
                            outArgs(tmpr);
                            out(DoIt2(cons, mtd, tmpr, MethodArgs, v));
                        } else {
                            outArgs(args);
                            out(DoIt2(cons, mtd, tmpr, new Double(1), v));
                        }
                    }
                }
            } else {
    ...
    public static void outArgs(Object[] obj) {
        System.out.println("\nSubmit: ");
        for (int a = 0; a < obj.length; a++) System.out.print(" " + obj[a]);
    }
    ...
    public static String DoIt2(Constructor cons, Method mtd, Object[] args, Object[] MethodArgs, int v) {
        try {
            mtd.invoke(cons.newInstance(args), MethodArgs);
            return "No Problem";
        } catch (Exception e) {
            FuzzerMethod.exceptionsThrown++;
            if (v == 1) {
    ...
    public static String DoIt2(Constructor cons, Method mtd, Object[] MethodArgs, int one, int v) {
        try {
            mtd.invoke(cons.newInstance(), MethodArgs);
            return "No Problem";
        } catch (Exception e) {
            FuzzerMethod.exceptionsThrown++;
            if (v == 1) {
    ...
    public static String DoIt2(Constructor cons, Method mtd, Object[] args, double two, int v) {
        try {
            mtd.invoke(cons.newInstance(args));
            return "No Problem";
        } catch (Exception e) {
            FuzzerMethod.exceptionsThrown++;
            if (v == 1) {
